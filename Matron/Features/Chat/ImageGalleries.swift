...
/// `MatronMac/Features/Chat/ImageGalleries.swift` token-for-token — the
/// two app targets have separate `AppDependencies` types, so this cannot
/// live in a shared package.
enum ImageGalleries {
    /// Gallery for a chat-timeline tap: every image in the conversation,
    /// oldest → newest (so → / swipe-left means "newer", matching the
    /// direction the timeline reads), starting at the tapped one.
    ///
